...
const EmployeeTarget = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetching only Employees
        const employeeRes = await api.get("/employee");

        setEmployees(employeeRes.data?.employee || []);

        alertShownRef.current.employees = false;
      } catch (err) {
        console.error("Error fetching employees:", err);

        if (!alertShownRef.current.employees) {
          setAlertConfig({
            visibility: true,
            message: "Failed to load employees",
            type: "error",
          });
          alertShownRef.current.employees = true;
  ...
  useEffect(() => {
    ...
    const fetchTargets = async () => {
      try {
        ...
        // Fetching Employee Targets
        const res = await api.get("/target/employee", {
          params: {
            from_date,
            to_date,
          },
  ...
  }, [selectedDate, initialDataLoading, reload]);
  ...
  const fetchTargetDetails = async (item) => {
    try {
      ...
      // Note: Keeping original endpoint logic for employees
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isBulkMode) {
        // Bulk update for Employees
        // Using agentIds key as per original code logic to preserve functionality
        const agentIds = employees.map((e) => e._id);
      ...
      <div className="flex mt-20">
        ...
        <div className="flex-grow p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-semibold">Employee Targets Management</h1>
            <button
            ...
            <>
              ...
              <div className="relative min-h-[200px]">
                ...
                <DataTable
                  data={tableData}
                  columns={getColumns()}
                  exportedPdfName="Employee-Target-Report"
                  exportedFileName="Employee-Target-Report.csv"
                />
              </div>
            </>
          )}
