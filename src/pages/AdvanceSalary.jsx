...
import { Select, Dropdown, Drawer, Modal } from "antd";
import { IoMdMore } from "react-icons/io";
import BackdropBlurLoader from "../components/loaders/BackdropBlurLoader";
import CircularLoader from "../components/loaders/CircularLoader";
import { numberToIndianWords } from "../helpers/numberToIndianWords";
import { FiUser, FiCalendar, FiDollarSign, FiCreditCard, FiHash } from 'react-icons/fi';
...
const EmployeeAdvancePayment = () => {
          ...
          <Dropdown
            menu={{
              items: [
                ...
                { key: "2", label: <span className="text-red-500">Delete</span>, onClick: () => showDeleteConfirm(item._id) },
              ],
            }}
            trigger={["click"]}
          >
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setAlertConfig({ visibility: true, message: "Record deleted", type: "success", noReload: true });
      fetchAdvanceHistory();
    } catch (error) {
      setAlertConfig({ visibility: true, message: "Delete failed", type: "error", noReload: true });
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlertConfig({ visibility: true, message: editId ? "Updated" : "Added", type: "success", noReload: true });
      closeDrawer();
      fetchAdvanceHistory();
    } catch (error) {
      setAlertConfig({ visibility: true, message: "Error saving data", type: "error", noReload: true });
    } finally {
      setOpenBackdropLoader(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.amount || formData.amount <= 0) errs.amount = "Enter valid amount";
      ...
      <div className="flex mt-20">
        ...
        <div className="flex-grow p-7">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowModal(true)} className="bg-blue-950 text-white px-4 py-2 rounded">+ Add Advance</button>
          </div>

          <DataTable
            data={tablePayments.filter(item => item.employeeName?.toLowerCase().includes(searchText.toLowerCase()))}
            columns={columns}
          />
        </div>
    ...
    {formData.pay_type !== 'cash' && (
      <div className="space-y-2 animate-fadeIn">
        <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
          ...
          {formData.pay_type === 'cheque' ? 'Cheque Number' : 'Transaction ID'}
        </label>
        <input
          type="text"
          className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
          placeholder={formData.pay_type === 'cheque' ? 'Enter cheque number' : 'Enter transaction/UPI ID'}
