...
import { DollarCircleOutlined, TeamOutlined, TrophyOutlined, MinusCircleOutlined, PlusCircleOutlined } from "@ant-design/icons";
...
const EmployeeStatement = () => {
  ...
  const fetchLedgerData = async (employeeId) => {
    try {
      setIsLoading(true);
      const response = await api.get(`/finance/ledger/${employeeId}`);
      ...
      const formattedData = (response.data.ledger || []).map((item, index) => {
        ...
        return {
          ...
          debit: item.debit > 0 ? item.debit.toLocaleString('en-IN', { 
            style: 'currency', 
            currency: 'INR',
            minimumFractionDigits: 0,
            maximumFractionDigits: 0
          }) : '-',
          credit: item.credit > 0 ? item.credit.toLocaleString('en-IN', { 
            style: 'currency', 
            currency: 'INR',
            minimumFractionDigits: 0,
            maximumFractionDigits: 0
  ...
  const getIncentiveTag = (adjustment) => {
    ...
    if (adjustment > 0) return <Tag icon={<TrophyOutlined />} color="green">+₹{adjustment.toLocaleString()}</Tag>;
    return <Tag icon={<MinusCircleOutlined />} color="red">-₹{Math.abs(adjustment).toLocaleString()}</Tag>;
  };
          ...
          <div className="flex-grow p-7">
                ...
                <div className="w-full sm:w-auto">
                  <Select
                    ...
                    options={employees.map(emp => ({
                      value: emp._id,
                      label: `${emp.name} ${emp.phone_number}(${emp.employeeCode || 'N/A'})`,
                      employee: emp
                    }))}
                  />
                </div>
              ...
              {selectedEmployee && (
                <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <Space>
                    ...
                    <Text strong>
                      Showing ledger for: <span className="text-blue-900">{employees.find(e => e._id === selectedEmployee)?.name}</span>
                    </Text>
                  </Space>
                </div>
              )}
            ...
            {selectedEmployee ? (
                  ...
                  <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                      <div className="p-4 bg-white rounded-lg shadow border border-gray-100">
                        <Text type="secondary" className="text-sm">Total Credits</Text>
                        <Title level={3} className="text-green-600 mb-0 mt-1 flex items-center">
                          <PlusCircleOutlined className="mr-2" />
                          {ledgerData.reduce((sum, item) => sum + item.credit, 0).toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            minimumFractionDigits: 0,
                            maximumFractionDigits: 0
                      ...
                      <div className="p-4 bg-white rounded-lg shadow border border-gray-100">
                        <Text type="secondary" className="text-sm">Total Debits</Text>
                        <Title level={3} className="text-red-600 mb-0 mt-1 flex items-center">
                          <MinusCircleOutlined className="mr-2" />
                          {ledgerData.reduce((sum, item) => sum + item.debit, 0).toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            minimumFractionDigits: 0,
                            maximumFractionDigits: 0
                      ...
                      <div className="p-4 bg-white rounded-lg shadow border border-gray-100">
                        <Text type="secondary" className="text-sm">Current Balance</Text>
                        <Title level={3} className={`mb-0 mt-1 flex items-center ${
                          ledgerData[ledgerData.length - 1]?.balance > 0 ? 'text-green-600' : 'text-red-600'
                        }`}>
                          ...
                          {Math.abs(ledgerData[ledgerData.length - 1]?.balance).toLocaleString('en-IN', {
                            style: 'currency',
                            currency: 'INR',
                            minimumFractionDigits: 0,
                            maximumFractionDigits: 0
              ...
              <div className="border-2 border-dashed rounded-xl p-12 text-center text-gray-500 bg-gray-50">
                ...
                <Title level={4} className="text-gray-700">Select an Employee</Title>
                <Text className="mt-2 block">
                  Use the dropdown above to choose an employee and view their detailed salary ledger
                </Text>
              </div>
            )}
        ...
        <Drawer
          title={
            <div className="flex items-center">
              ...
              <span className="font-semibold">{currentDetails?.month} Details</span>
            </div>
          }
          width={650}
          onClose={closeDrawer}
            ...
            <div className="space-y-6">
                ...
                <Descriptions title="Monthly Summary" column={1} bordered>
                  <Descriptions.Item label="Status">
                    <Tag color={currentDetails.balance >= 0 ? "green" : "red"}>
                      {currentDetails.balance >= 0 ? "Credit Balance" : "Debit Balance"}
                    </Tag>
                  </Descriptions.Item>
                  <Descriptions.Item label="Net Amount">
                    <Title level={4} className={currentDetails.balance >= 0 ? "text-green-600" : "text-red-600"}>
              ...
              <Descriptions title="Salary Components" column={1} bordered>
                <Descriptions.Item label="Base Salary">
                  <Text strong>
                    {currentDetails.details.base_salary.toLocaleString('en-IN', {
                      style: 'currency',
                      currency: 'INR',
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0
                ...
                <Descriptions.Item label="Other Payments">
                  <Text type="success" strong>
                    + {currentDetails.details.other_payments.toLocaleString('en-IN', {
                      style: 'currency',
                      currency: 'INR',
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0
                ...
                <Descriptions.Item label="Other Deductions">
                  <Text type="danger" strong>
                    - {currentDetails.details.other_deductions.toLocaleString('en-IN', {
                      style: 'currency',
                      currency: 'INR',
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0
              ...
              <Descriptions title="Business Performance" column={1} bordered>
                <Descriptions.Item label="Target Achievement">
                  {getTargetAchievementTag(currentDetails.details.target_achievement)}
                </Descriptions.Item>
                <Descriptions.Item label="Business Closed">
                  <Text strong>
                    {currentDetails.details.business_closed.toLocaleString('en-IN', {
                      style: 'currency',
                      currency: 'INR',
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <div className="space-y-2 text-gray-700">
                  <p>
                    <span className="font-medium">Base Amount:</span> ₹{currentDetails.details.base_salary.toLocaleString()}
                  </p>
                  <p>
                    <span className="font-medium">+ Other Payments:</span> ₹{currentDetails.details.other_payments.toLocaleString()}
                  </p>
                  <p>
                    <span className="font-medium">+ Incentive:</span> {currentDetails.details.incentive_adjustment > 0 ? '+' : ''}₹{Math.abs(currentDetails.details.incentive_adjustment).toLocaleString()}
                  </p>
                  <p>
                    <span className="font-medium">- Other Deductions:</span> -₹{currentDetails.details.other_deductions.toLocaleString()}
                  </p>
                  <div className="border-t pt-2 mt-2">
                    <p className="font-semibold">
                      Net Amount: {currentDetails.balance >= 0 ? '+' : '-'}₹{Math.abs(currentDetails.balance).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center text-sm text-gray-500 mt-4">
                <p>Data as of {moment().format('MMMM DD, YYYY')} | Calculation based on company policy</p>
              </div>
            </div>
          )}
        </Drawer>
